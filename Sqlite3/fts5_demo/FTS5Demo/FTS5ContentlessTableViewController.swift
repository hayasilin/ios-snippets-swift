...
final class FTS5ContentlessTableViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        searchDatabase = configureFTS5ContentlessDatabase()
        createSearchTable()
    ...
    private func createSearchTable() {
        let sqlQueryString = "CREATE VIRTUAL TABLE IF NOT EXISTS \(ftsTable.ftsTableName) USING fts5(content='', text);"
    ...
    private func insertMainTable(with text: String) -> (text: String, messageID: Int32) {
        let sqlQueryString = "INSERT INTO \(ftsTable.mainTableName)(id, text, message_id) VALUES (?, ?, ?);"
        var statement: OpaquePointer?
